...
test("DocsReader normaliza el contexto invalidado cuando enviarMensaje falla sincronicamente", async () => {
...
test("DocsRuntime normaliza lastError del callback sin dejar rechazo no manejado", async () => {
  ...
  const error = await exports.DocsRuntime.enviarMensaje({ type: "PING" }).catch(
    (runtimeError) => ({
...
test("DocsRuntime.obtenerURL devuelve null cuando el contexto de la extension fue invalidado", () => {
  const { exports } = createSandbox({
    getURL() {
      throw new Error("Extension context invalidated.");
    },
  ...
  const resolvedUrl = exports.DocsRuntime.obtenerURL("assets/icons/logo.svg");
