...
5. The extension auto-initializes when you open a Google Docs document
...
- `Orchestrator` object: Main entry point that initializes the extension
...
Each rule lives in its own folder. `rules/<rule-id>/rule.ts` exports a typed rule object, optional dictionary data lives in `rules/<rule-id>/patterns.json`, and rule-specific tests live in the same folder. `rules/index.ts` aggregates the exported rules:
    ...
    return [
      {
        ...
        sugerencias: ["suggested replacement"],
        regla: "rule-id",
...
`rules/index.ts` is the source of truth for registration order. Runtime imports should point at `./rule-id/rule.js` and keep `.js` specifiers so TypeScript and esbuild resolve them consistently. Both regex patterns use case-insensitive matching with word boundaries (`\b`) by default.

### 5. **UI Panel** (`content/panel.js`, `panel/`)

- `DocsPanel` object: Manages the floating sidebar
- Builds the panel markup in `content/panel.js` and applies styles from `panel/panel.css`
...
1. Create `rules/new-rule/` in the rules directory
2. Define the rule object in `rules/new-rule/rule.ts` (see Rule System section above)
3. Put curated dictionary data in `rules/new-rule/patterns.json` when the rule is dictionary-driven
4. Add `rules/new-rule/new-rule.test.js`
5. Import it in `rules/index.ts` from `./new-rule/rule.js` and append it to the exported `rules` array
...
### Current Limitations
 from the Docs API remains the source of truth for issue offsets
- The overlay does not trust raw DOM positions directly; it normalizes visible text and maps issue text back into `Range` rects
- **Bug zone**: If the accessibility layer diverges from the API text, some issues may stay panel-only until the mapper is adjusted
...
- If highlights don't appear: verify the rule is exported in `rules/index.ts` and positions are correct
