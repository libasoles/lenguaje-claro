...
async function debugGoogleDocs() {
  ...
  try {
    ...
    await page.goto('https://docs.google.com/document/d/1', { waitUntil: 'networkidle' });
    ...
    const diagnostico = await page.evaluate(() => {
      ...
      results.appViewKixApp = document.querySelectorAll('.kix-appview-edit-container').length;
      ...
      results.klassicSelectors = {
        ...
        'data-paragraph-id': document.querySelectorAll('[data-paragraph-id]').length,
      };

      // 3. Buscar docos-stream-view
      const streamView = document.querySelector('[role="list"].docos-stream-view');
      results.streamViewFound = !!streamView;

      if (streamView) {
        results.streamViewText = streamView.innerText?.substring(0, 100) || 'Sin texto';
      ...
      const allElements = document.querySelectorAll('[class*="kix"], [class*="doc"]');
      ...
      const editContainer = document.querySelector('[role="region"][aria-label*="ument"]');
    ...
    console.log(`   - .kix-appview-edit-container: ${diagnostico.appViewKixApp}`);
    ...
    if (diagnostico.streamViewFound) {
      ...
      if (diagnostico.firstChildren) {
        ...
        diagnostico.firstChildren.forEach((child, i) => {
          console.log(`     ${i}: <${child.tag}> clase="${child.class}" role="${child.role}" data-id="${child.dataId}"`);
    ...
    const textExtraction = await page.evaluate(() => {
      ...
      try {
        ...
        result.methods['docos-stream-view.innerText'] = { success: false, error: e.message };
      }
      ...
      try {
        ...
        result.methods['document.body.innerText'] = { success: false, error: e.message };
      }

      // Método 3: Recolectar todos los nodos de texto
      try {
        const walker = document.createTreeWalker(
          document.querySelector('[role="list"].docos-stream-view') || document.body,
          NodeFilter.SHOW_TEXT,
          null
        );
        const texts = [];
    ...
    Object.entries(textExtraction.methods).forEach(([method, data]) => {
      if (data.success) {
        console.log(`✅ ${method}`);
        console.log(`   Longitud: ${data.length || data.nodeCount} caracteres/nodos`);
    ...
    console.log('\n\n✨ Debug completado. Revisor de Escritura debería estar cargado en la página.');
    console.log('📋 Revisar la consola del navegador para logs de [Docs Reviewer]\n');

  } catch (error) {
    console.error('Error durante debugging:', error);
  } finally {
    // Mantener el navegador abierto por 30 segundos para inspección
    console.log('Navegador permanecerá abierto por 30 segundos para inspección...');
