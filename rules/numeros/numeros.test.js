...
test("numeros detecta cardinales compuestos y magnitudes completas", () => {
  const rule = loadRule("numerosRule", "./rules/numeros/rule.js");
  ...
  assert.deepEqual(
    Array.from(findings, (item) => [item.textoOriginal.toLocaleLowerCase(), item.sugerencias[0]]),
    [
      ["treinta y dos", "32"],
      ["ciento veinte", "120"],
      ["mil doscientos", "1200"],
...
test("numeros soporta variantes frecuentes con y sin tilde", () => {
  ...
  assert.deepEqual(
    Array.from(findings, (item) => [item.textoOriginal, item.sugerencias[0]]),
    [
      ["veintiún", "21"],
      ["doscientas", "200"],
      ["dieciseis", "16"],
...
test("numeros ignora expresiones que ya incluyen el digito entre parentesis", () => {
  const rule = loadRule("numerosRule", "./rules/numeros/rule.js");
  const findings = rule.detectar("Tiene treinta (30) cuotas y cuarenta cuotas.");

  assert.deepEqual(
    Array.from(findings, (item) => [
      item.textoOriginal,
      Array.from(item.sugerencias),
    ]),
