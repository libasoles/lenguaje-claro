...
const STATIC_TARGETS = [
  ...
  {
    from: path.join(ROOT, "rules", "patterns.json"),
    to: path.join(DIST, "rules", "patterns.json"),
  },
];
...
const STATIC_WATCH_PATHS = [
  ...
  path.join(ROOT, "rules", "patterns.json"),
];
...
function readEnvFile() {
  ...
  if (!fs.existsSync(envPath)) {
    throw new Error(
      "No se encontró .env. Copiá .env.example a .env y definí CHROME_OAUTH_CLIENT_ID.",
    );
  }
